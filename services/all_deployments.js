...
async function getAllDeployments() {
    ...
    try {
        ...
        try {
            // Get an array of all badges deployers addresses 
            const all_badges_deployers = await badgeFactoryContract.get_all_badges_deployers()
            // console.log(all_badges_deployers)

            for(let addIndex = 0; addIndex < all_badges_deployers.length; addIndex++) {
                // For this deployer, find their total deployments
                const totalDeploys = await badgeFactoryContract.get_total_badges_deployments_by_owner(all_badges_deployers[addIndex])
                const totalDeploys_int = parseInt(totalDeploys["_hex"], 16)

                for(let i = 0; i < totalDeploys_int; i++) {
                    // For each deployment, get the address
                    const badges_deployed_at = await badgeFactoryContract.get_nth_badges_contract_address(all_badges_deployers[addIndex], i)
            ...
            console.log(e)
            console.log("Couldn't get all badges deployers")
        }
    } catch (e) {
